...
from Structural_Functions import simulate_series,ploty,compute_irf,plot_irf_with_confidence_intervals, ploty_old,analyze_pca,select_blocks, adf_test, add_double_lines_to_latex,generate_blocks, Correlogram,  Companion_Matrix_compute, breusch_test, histogram_Normality_test
...
#PCA 
PC, Principal_Components = analyze_pca(climate_data.drop('E3CI',axis=1),threshold= 0.90,n_components=4,standardize=0)
...
PC_instrument, Principal_Components_instrument = analyze_pca(climate_data_instrument ,threshold= 0.90,n_components=4,standardize=0)
loadings_instrument = PC_instrument.components_
print(loadings_instrument)
...
df_macro_eurostat = pd.read_excel(f"{path}\Working Data_EUROSTAT.xlsx" , index_col=0).dropna().drop('Euro_Short_Rate', axis =1).astype(float)
...
Var_data = Var_data.drop('Uncertainty_Italy_News', axis=1).drop('Iprod_Italy_Year_on_year_growth', axis=1)#.drop('HICP_FE', axis= 1)
...
with timer('Var cell'):
    adf_test(Var_data)
    Sigma_u, AR_matrices, Residuals,Non_ar_params = estimate_var_model_const_trend_ols_boot(Var_data, p=2, noprint=False,returnconst=True)#If norpint is false then summary and correlation matrix of residuals is printed
...
Structparams = [k_hat,corr_instr,rho_hat_std,rhosqu_hat, phi_hat, eigenvalues, Companion_Matrix]

del k_hat,corr_instr,rho_hat_std,rhosqu_hat, phi_hat, eigenvalues,Companion_Matrix, IRF_hat_h
#%% Bootstrap cell (safe and easy, with vectorization)
#README: choose whether to choose this cell or the one below, this one is widely compatible but slower.
...
#     for i in range(p,T): Sample_boot[i] = Non_ar_params[:,0].T + sum(AR_matrices[j] @ Sample_boot[i - j - 1].T for j in range(0, p)) + Residuals_boot[i-2,:]           
...
#     if k_hat_boot[0] < 0 : k_hat_boot=k_hat_boot*(-1), print('correction')
...
#     nboot = 1000 # Number of boot iterations
...
#%% Parallel bottstrap cell
# WARNING: if you decide to run the code in this cell, please manually select the strings and manually run them after you run the previous code. Many interpreters might bug if you use "run this cell" or similar commands.
# e.g use ctrl+f9 in spyder or copy this code in the console command 
#Personally I reccomend to run this if you want faster code but otherwise run the numpy vectorization so you can do something else in the meanwhile
with timer('Boot cellparallel'):
    block_length = 12
    Residuals_expanded = np.hstack((Residuals, instrument[p:]))  # we stack all residuals together to jointly boot them
    blocks = generate_blocks(Residuals_expanded, block_length=block_length)
    nboot = 1000 # Number of boot iterations, 1000 is set for a quick check, replace with 20000 to replicate results
    IRF_matrices_boot = np.zeros((nboot, hor + 1, M))  # 3D array to store Bootstrap results
    Sample_boot = np.zeros((T + p, M))  # We Initialize the sample
    Sample_boot[:p, :] = Var_data.iloc[:p].to_numpy()  # makes first p values equal to the original sample
...
with timer('projection cell'):
    ...
    Z = instrument.to_numpy().reshape(-1,1) #we make the instrument a 2d array
    VD = Var_data.to_numpy() # we turn var data into numpy array for faster execution
    const = np.ones((len(VD[p:,0]),1))
    trend = np.arange(2,len(VD[p:,0])+2).reshape(-1, 1)
    X_vars_lag1 = np.delete(VD[p-1:T + p-1, :], [0], axis=1)
    X_vars_lag2 = np.delete(VD[p-2:T + p-2, :], [0], axis=1)
    instrument_lag1 = Z[p-1:T+p-1]
    instrument_lag2 = Z[p-2:T+p-2]
    X = np.hstack([X_vars_lag1,X_vars_lag2,instrument_lag1,instrument_lag2,Z[p:],const,trend])
    E3 = sm.OLS(VD[p:,0],X).fit(cov_type='HC3').fittedvalues
    p_val = np.zeros((len(grid),hor+1,M ))
    for i in np.arange(0,len(grid)):
        for y in np.arange(0,M):
            for h in np.arange(0,hor+1):
                # time t variables
                Y = VD[h+p:, y] - grid[i]*E3[h:]
                X_vars_lag1 = np.delete(VD[p-1:T + p-1 - h, :], [0, y], axis=1)
                X_vars_lag2 = np.delete(VD[p-2:T + p-2 - h, :], [0, y], axis=1)
                const = np.ones((len(Y),1))
                trend = np.arange(2,len(Y)+2).reshape(-1, 1) # creates the constant as a list of integers increasing by 1 at a time.
                instrument_lag1 = Z[p-1:T+p-1-h]
                instrument_lag2 = Z[p-2:T+p-2-h]
                X = np.hstack([X_vars_lag1, X_vars_lag2,instrument_lag1,instrument_lag2,
                               Z[p:T+p-h],trend, const ]) #this makes it so that instrument is at the column M-1 or M-2, implying also his coefficient will be.
                model = sm.OLS(Y,X).fit(cov_type='HC3')
                contrast_matrix = np.zeros(len(model.params))
                contrast_matrix[len(model.params)-3] = 1
                t_test = model.t_test(contrast_matrix)
                p_value = t_test.pvalue.item()
                p_val[i,h,y] = p_value
                
    #we now move on to plot the implied IRFs
    lower_bounds_90 = np.zeros((hor+1, M))
    upper_bounds_90 = np.zeros((hor+1, M))
    lower_bounds_68 = np.zeros((hor+1, M))
    upper_bounds_68 = np.zeros((hor+1, M))
    central_points = np.zeros((hor+1, M))
    ...
    for y in range(M):
        for h in range(hor+1):
            ...
            central_points[h, y] = grid[max_pval_index] # we map back our p-val to the grid value
            
    for y in range(M):
                plot_irf_with_confidence_intervals(
                    np.arange(hor+1), 
                    central_points[:, y], 
                    lower_bounds_90[:, y], 
                    upper_bounds_90[:, y], 
