...
climate_data = climate_data.drop('fire', axis=1).drop('hail',
                                                      axis=1)  # We drop those 2 to conform withne IFAB approach an
# drop E3CI since it is computed on an older value
...
def MBB(b,
        IRF_matrices_boot):  # this is the function that will perform the bootstrap. If you prefer you can run a parallel version of it by using the section below.
    if yesprint == True:
...
with timer('projection cell'):
    ...
    if use_weak == 0:
        grid = np.arange(-1, 1.2, 0.001)
    elif use_weak == 1:
        grid = np.arange(-5, 5, 0.10)
        ...
        for y in np.arange(0, M):
            for h in np.arange(0, hor + 1):
                ...
                trend = np.arange(p + 1, len(Y) + p + 1).reshape(-1,
                                                                 1)  # creates the trend as a list of integers increasing by 1 at a time.
